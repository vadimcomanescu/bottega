...
describe("setup v2 semantic reconciliation", () => {
  ...
  it("gives domain consumers one owner and migrates real material", () => {
    ...
    expect(setup).toContain("A new Bottega-owned layout uses the relevant `CONTEXT.md`.");
    expect(read("skills/discover/SKILL.md")).toContain("Treat genuinely absent domain material as absent, not as a setup requirement.");
    expect(read("skills/improve/SKILL.md")).toContain("Treat genuinely absent domain material as absent, not as a setup requirement.");
  });

  it("keeps GitHub claims, triage, and landing conditional on repository evidence", () => {
    expect(setup).toContain("Bottega routes only to GitHub and does not import a tracker or triage engine.");
