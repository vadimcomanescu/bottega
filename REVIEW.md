...
- Model selection lives in `skills/routing`: the model table and the task rules. A skill that restates them or pins a model elsewhere is a defect, with one sanctioned exception: the review gate's engine pins in `skills/autoreview`. Callers say "invoke bottega:routing".
