...
Follow bottega's `skills/reviewing/SKILL.md` (under `$CLAUDE_PLUGIN_ROOT` when installed as a plugin; the repo root inside bottega itself) to the letter: pass 1 break it (reproduced failures outrank arguments), pass 2 test ratchet (any skipped/weakened/deleted test is critical blocking), pass 3 architectural conformance against the dossier's interface contract, judged in the shared vocabulary of `skills/designing/SKILL.md` (same root).
