...
Read the target whole: the named doc, or the repo's agent surface (the root `CLAUDE.md` or `AGENTS.md`, and the skills and rules files it routes to), together with what else loads for the same reader, because a fact's second home only shows when both homes are in view. Vendored and third-party text is out of scope. You are done reading when everything in scope is read.

Then judge. Every sentence gets one of three outcomes, and every cut names the test that fired:

- **Cut** when it fails one of these: the model does it unprompted (no-op), the repo, plan, or artifact already carries it (derivable), an adjacent sentence already says it (restatement), another file loaded by the same reader states it (second home), or it dictates keystrokes where the outcome would do (mechanism-scripting).
