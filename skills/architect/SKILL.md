...
- When a consequential interface has no strong repository precedent or standard solution deciding it, design it twice before committing. Dispatch two or three parallel Opus workers, each a fresh context given the same brief and one design pressure to draft under, chosen so the drafts come out genuinely different: the smallest possible surface, the shape the most common caller wants, the shape the seam's dependency category suggests, or whatever axis this interface actually varies on. Compare the drafts on depth, locality, and seam placement, and pick one with a stated reason. The first design is rarely the best, and a doctrine that favors deep modules makes it easy to stop at the first deep-looking one.
