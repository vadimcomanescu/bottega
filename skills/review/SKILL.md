...
- The listed inputs are the default review axes. Before dispatch, examine the frozen diff and add to the intent file any review angle this diff needs that the defaults do not name (a migration's rollback, a concurrency surface, a permission boundary), then state in the adjudication which added angles applied.
