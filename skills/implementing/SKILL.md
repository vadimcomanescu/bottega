...
- Before adding code, stop at the first correct option: the behavior need not exist; the codebase already has it; the standard library has it; the platform has it; an installed dependency has it; one clear direct expression does it; only then write the minimum new code, shaped the way the technology's current documentation and the supplied skills prescribe.
...
- The approved plan and repository remain authoritative over any technology skill supplied with the dispatch. Verify version-sensitive APIs against the installed version and primary vendor documentation. A plan step that contradicts the technology's documented practice is a conflict to raise with the orchestrator before building, not a choice to make silently.
- Stay inside the slice. Do not implement the next task or unrelated cleanup. A changed test must name the plan requirement that changed it; a skipped or weakened test is not green.
