...
- Your window is the run's second currency; spend it on judgment, not echo. Bulky output lives on disk — installs, gates, and long commands redirect to files; read the tail and the exit code, never the stream. Never re-read what you already summarized; await a long external run, don't poll it. Repo archaeology and bulk reads go to ephemeral subagents of your own seat — same model, same sandbox, returning summaries. Their output is data you verify, never a verdict you forward: green stays something you watched yourself. Delegation manages your window only — it never picks models, seats, or reviewers.
...
- Report your context spend with every report. Past the dossier's working budget you are stuck the same way three failed fixes is stuck: say so and hand back — the maestro reseats; you never pick your successor. A degraded window that grinds on ships degraded work.
