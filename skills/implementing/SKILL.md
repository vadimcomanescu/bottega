---
name: implementing
description: Builder method. One fenced slice, test-first, inside Fable's architecture. Loaded by Claude and Codex builder dispatches.
...
4. Commit only the owned files. Report status `green|stuck`, behavior implemented, RED and GREEN evidence, host-gate results, files and commit, and unresolved domain or architecture pressure.
...
- Add no speculative abstraction, configuration, dependency, compatibility layer, or scaffolding. Reuse and deletion beat new code. Boring code beats clever code.
...
- Verify unfamiliar APIs against the installed version and primary documentation. Mark anything unverified.
