...
| seat | dispatch |
| --- | --- |
| codex | one read-only codex dispatch per `use-codex`: gpt-5.6-sol at max effort, the brief asking it to ground the draft on current web sources, and the report you read back saved as `<session>/codex-draft.md` |
