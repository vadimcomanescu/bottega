...
1. **Isolation.** Branch `bottega/patch-<slug>` in a worktree at `.bottega/wt/patch-<slug>/`; write `$CLAUDE_CODE_SESSION_ID` (your own shell's, never a seat's) to `.bottega/run/patch-<slug>/owner` so the route guard's run fence binds to this session and no concurrent one. The user's checkout stays on trunk, and the PR is the only path there.
...
4. **Deliver.** A PR: what was broken or lacking, what changed, the test that holds it, provenance (builder and reviewer, family and model, rounds, findings and verdicts), and any call the ask underdetermined, made and flagged. Push, open the PR, reap the worktree and `.bottega/run/patch-<slug>/` (the pushed branch holds everything); delete the local branch after the merge.
