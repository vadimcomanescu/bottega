...
QA is done when every changed surface has a verdict, its evidence, and its recording (or the stated reason it could not be recorded). Recordings publish from the never-merged branch `bottega/evidence-<slug>` so they render in the PR. The branch dies after merge; the evidence's job ends when the user merges.
