...
const panelistTask = `You are one independent panelist. Work from this prompt and the repository alone. Give a complete answer without guessing what another panelist may say.\n\nTask:\n${args.task}`
const [a, b] = await parallel([
  ...
  // Panelist B: fable at high.
  () => agent(panelistTask, { label: 'panelist:fable', model: 'fable', effort: 'high', schema: PANELIST }),
])
...
phase('Judge')
const judge = await agent(`You compare blinded drafts. Answer only the requested comparison, quote the drafts as evidence, and do not merge them or answer the task yourself.\n\n${blinded}\n\nCompare draft A and draft B.`,
  { label: 'judge', model: 'fable', effort: 'high', schema: JUDGE })
