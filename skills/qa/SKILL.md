...
4. Record the drive itself, one recording per scenario, into `.bottega/verify/<feature-slug>/recordings/`: a browser flow runs inside `agent-browser record start <path>` … `record stop` (WebM); a CLI flow inside a terminal recorder (asciinema or vhs — whichever the dossier names or the seat verifies present); a desktop flow under screen capture. A recording is evidence only if it is the session that produced the verdict — a re-staged demo is testimony. A surface with no capture channel is reported as unrecordable, never faked.
...
6. Write the full transcript into `.bottega/verify/<feature-slug>/qa-transcript.txt`, opening with the commit you drove. Touch nothing else in the repo; commit nothing. Completion: every scenario row has a verdict, its evidence, and its recording path (or the unrecordable reason).
