...
- Drive every supplied scenario through the interface a user or integrator actually uses, with the tool the surface calls for: your harness's browser tool for web (a scripted driver where the harness has none), computer use for a desktop app, on the local machine only, a real process run for a CLI. A fixture or demo may set up the drive, but the verdict comes from behavior observed through that interface, never from code inspection or a screenshot staged after the run.
