...
- Drive every supplied scenario through the interface a user or integrator actually uses, with the tool the surface calls for: agent-browser for web, computer use for desktop, a real process run for a CLI. A fixture or demo may set up the drive, but the verdict comes from behavior observed through that interface, never from code inspection or a screenshot staged after the run.
