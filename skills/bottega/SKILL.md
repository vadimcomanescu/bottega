...
1. Read the code first — and `docs/specs/`, where closed specs carry the decisions and dead ends of every prior run; what failed there before is part of the territory. Ask at most three questions — only what the request genuinely underdetermines; scale question count to task size (bug fix: 0–1, greenfield: a handful).
...
**Build.** Dispatch implementors with self-contained dossiers: slice intent, red tests, interface contract, owned files, dead ends closed specs record for this territory, and the instruction to follow `skills/implementing`. One task per invocation — the worker commits, reports, stops.
...
**Close.** After delivery, rewrite the spec from a build plan into a durable record: outcome, what shipped where (pointers at code and `.bottega/verify/<sha>/`), and the decisions as rationale — the parts that stay true after the code moves on. Record the dead ends beside them: approaches tried and failed anywhere in the run, each with the constraint it hit — facts, not prescriptions, so they never go stale, and future dossiers inherit them. Mark it closed; it is history now, not operational truth.
