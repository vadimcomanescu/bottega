...
The launch decided the release. Read the run's recorded answer, `.bottega/run/<slug>/release`, for whether the PR lands on green or holds for me. Never merge a PR by hand and never approve one. Arm auto-merge on the PR you open where the repository's own merging procedure makes arming the opener's act. Let the repository's required checks and the `hold` label decide whether an armed PR lands. Review feedback that arrives after the PR opens goes through `bottega:code-review`.
...
When the answer is hold, make the label exist first (`gh label create hold --description "Blocks the merge until a person removes it" || true`). Add `--label hold` on the same create call so the PR opens carrying it. Then confirm the repository enforces the label on this PR in whichever shape it uses: a merge queue that refuses the PR while the label is present, named in the queue's own summary on the PR with the checks still green, or a required check that is red because the label is present. When neither is there, stop the run here and report it to me, because the PR would say it is held while it is still free to merge.
...
- Only a person clears it (a required review, a label the project's rules ask a person to add): report what is needed and on which PR, and leave the PR open and unmerged for them. Never add it yourself and never remove a `hold` label, because both defeat a check that exists to put a person in the loop.
...
Name the PR, the head SHA, and the evidence links in every report. Merged (`state` is `MERGED`): report it and sweep the run's working state (the run directory, the worktree, the branch) from the project's main checkout, never from inside the worktree. Ready (checks green, merge state `CLEAN`): report it and say what lands it, reading that from `gh pr view <PR> --json autoMergeRequest` rather than guessing. A null where you armed it means the arm did not take, so re-arm. Held: report that the PR lands when I remove the `hold` label, which only I do. Waiting on a person: report the action needed and leave the PR open.
