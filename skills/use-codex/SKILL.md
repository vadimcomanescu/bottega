...
- `--full-access` when the job includes git writes (fetch, branch, commit, worktree). The write sandbox keeps every `.git` read-only and blocks the network, so a worker told to commit dies at its first git command. Full access removes the sandbox whole, filesystem and network.
