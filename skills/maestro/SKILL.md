...
Check your seat before anything else. A run is orchestrated from Claude Code on fable-5 at xhigh, held for the whole run. On any other model or harness, tell the user and recommend restarting there; continue only if the user says to continue.

Report progress where the user watches: the harness screen. Run single workers as subagents, and fan-out as a dynamic workflow, so everything running is visible there. Never launch a worker as a detached shell process: the harness cannot display it, and nothing reports back when it finishes.

Every dispatch names its model and effort on the call that starts the worker; the phase that makes a dispatch states its pin. Every Claude-side worker runs opus-5, with effort as the only cost lever: low for mechanical work (renames, doc sync, shell relays), medium for exploration, and the pins below for the rest. Two invariants bind every pin: fable-5 is this seat and never a worker, and no verification seat (the plan editor, the review engines, the conformance check) runs the family that built the diff. A GPT worker is one foreground CLI call through [references/codex-dispatch.md](references/codex-dispatch.md). Pins are defaults, not limits: a worker whose result fails its requirement gets one rerun at higher effort or on the other family after the failure is diagnosed, never automatic, never more than one step at a time.
