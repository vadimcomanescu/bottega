...
Claude models do the work and GPT models check it with fresh eyes. Each phase's skill states its worker's model and effort, and every dispatch names both on the call that starts the worker; the mechanical jobs you dispatch yourself (renames, doc sync, shell relays) go to opus-5 at low effort. A codex worker is a CLI call you run from your own turn per the codex dispatch method, [references/codex-dispatch.md](references/codex-dispatch.md); the dispatch site's model, effort, and sandbox override its defaults, and a brief carries the run's inputs by absolute path and names no slash command, subagent, or `$CLAUDE_PLUGIN_ROOT`, which a codex worker lacks. A worker that fails its requirement gets one rerun, at higher effort or on a GPT model, after you diagnose the failure; never automatic.
