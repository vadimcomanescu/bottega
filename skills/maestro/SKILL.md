...
Report progress where the user watches: the harness screen. A Claude worker runs there as a visible subagent and a codex worker as a tracked background task, so every worker holds a visible row for its whole run.
