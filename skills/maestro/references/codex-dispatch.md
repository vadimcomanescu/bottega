...
Launch it from a wrapper subagent, one per codex worker, so the worker holds a visible row for its whole run; the wrapper runs opus-5 at low effort. The wrapper makes one **foreground** call running the script, with an explicit timeout above the slice's expected runtime, and when the script exits it returns the out file's final message verbatim, or the exit code and stderr on failure, and nothing else. Raise the shell timeout ceiling so the foreground call can outlast a long build (bottega:setup writes it). Never `run_in_background`: a backgrounded dispatch inside a subagent is never delivered (docs/lessons/subagent-background-work-dies-silently.md), and a result reporting the command "was moved to the background" is a failed dispatch to report, not a run to wait for. The stall watchdog guarantees even a wedged run exits instead of sitting alive forever. Never a polling loop, never an untracked shell. A failed invocation is reported with its stderr, never worked around. Directory trust never gates these runs; a fresh, never-trusted worktree under `--ignore-user-config` executes normally.

Builders take `--sandbox danger-full-access`; consultation reads take `--sandbox read-only`. Full access is a decision, not a convenience; do not "fix" it back. Claude workers run unsandboxed on the same host, so the run's trust boundary is the trusted repo plus the worktree, and sandboxing only the codex workers adds no safety while breaking the worker: `workspace-write` denies every socket (no localhost bind, no dev server, no database, no integration suite) and denies shared-gitdir writes (no commits). Its one repair knob, `sandbox_workspace_write.network_access`, is silently ignored on macOS, so it cannot make a dispatch run identically on any host. If bottega ever runs on a repo the user does not trust, the sandbox comes back, for Claude and codex workers both, and `scripts/codex-exec` starts resolving the codex binary from absolute PATH entries only, so a checkout cannot shadow it.
...
A Codex builder is one dispatch that owns the whole assigned job, same as a Claude builder; the brief carries the run's commit message format. The worktree and branch are pre-created (a mechanical dispatch, or your own shell); nothing writes implementation code for the builder.
