...
Reconcile a machine and a repo with bottega, once. Configure Claude Code and the codex CLI the run's GPT cross-reads go through, then bring the project to the shape `bottega:architect` defines, its domain docs and its documentation architecture. Read that skill before you start the repo work.
