...
- **A merge-governance proposal** when the default branch has none, where the plan supports it: a ruleset with the project's gates as required checks, squash merges, and no bypass actors; auto-merge allowed and armed at PR open by the repo's own workflow with a fine-grained PAT, never `GITHUB_TOKEN`, so a token-originated merge still triggers the default branch's CI; a `hold` label enforced by its own cheap required check as the brake, so saying hold at launch or adding the label is the human act and removing it lands the PR (`docs/adr/0028-the-launch-decides-the-release.md`); the strict up-to-date requirement off, because auto-merge cannot update branches; required approvals stay zero while the owner authors the PRs, because an author cannot approve their own PR. Automation found merging a PR directly is shown as an edit that removes it.
