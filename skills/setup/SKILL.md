...
- The canonical agent map: root `AGENTS.md` and `CLAUDE.md`, their symlink state, the exact `<!-- bottega:setup v1 begin -->` or `<!-- bottega:setup v2 begin -->` block when either exists, and the map's routes. This is separate from `CONTEXT-MAP.md`, which maps bounded contexts and is not an agent map.
...
- **Canonical map.** When both root files are independent, ask which is canonical. When one symlinks the other, its target is the map. When neither exists, default to `CLAUDE.md` and let me veto it. Create the other filename as a symlink only when it has no material of its own.
