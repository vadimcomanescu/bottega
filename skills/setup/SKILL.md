...
For every gap between the found state and the shape, show the user the exact edit that closes it. Content moves, and you invent nothing: an empty glossary, ADR scaffold, or owner doc is nothing to write.
...
Report what you wrote, what was declined, and the findings that remain the user's to fix.

## Findings (the genuinely un-writable)

Report these and leave the fix to the user:
