...
- **Executable checks for the must-not-break flows** when the repo ships a user-facing surface: verify every flow the product cannot ship broken is exercised by an end-to-end check the suite can run and name on its own (a tag, or whatever naming its runner offers). Where a flow has no such check, draft the missing end-to-end specs in the suite's own conventions for the owner to correct before they land; where the harness cannot run that suite, file one issue naming the uncovered flows instead. The suite is where these flows are named, so no prose inventory of them is written (`docs/adr/0026-risk-scaled-phases-and-driven-slices.md`).
