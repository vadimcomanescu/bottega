...
- **A merge-governance proposal** when the default branch has none, where the plan supports it. GitHub's merge queue needs an organization-owned public repository or GitHub Enterprise Cloud, so the shape below stands in for it and accepts the risk GitHub's own table names: status checks may fail after the merge when the base moved incompatibly under a green PR, and the default branch's CI run is the catch. The shape: a ruleset with the project's gates as required checks, squash merges, and no bypass actors; auto-merge allowed and armed at PR open by the repo's own workflow with a fine-grained PAT, never `GITHUB_TOKEN`; the arming step treats GitHub's already-mergeable error as success (arming fails on a PR with nothing left to wait on, which is the outcome wanted) and reports every other failure, a missing or unusable PAT included, never swallowing it; a `hold` label enforced by its own cheap required check as the brake, its workflow triggered on `pull_request` types `opened, reopened, synchronize, labeled, unlabeled` and reporting a conclusion on every one, never a skip (the default trigger set omits `labeled` and `unlabeled`, so removing the label would never re-run the check, and a skipped required check satisfies no ruleset), so adding the label blocks the merge and removing it lands the PR; the strict up-to-date requirement off, which lets any required check that compares the head to the base pass against a base that has since moved, so a proposal that includes such a gate says how it re-derives the base or names the default branch's CI run as the catch; required approvals stay zero while the owner authors the PRs, because an author cannot approve their own PR. Automation found merging a PR directly is shown as an edit that removes it.
