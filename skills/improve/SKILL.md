...
**Deliver.** On approval, create one tracker issue carrying the findings, labelled for its area per the repository's conventions. Write for a reader who was not in this session: define or link every non-standard term. Then invoke `/bottega:deliver` on that issue. Improve never claims the issue; run acquires the claim itself. Report the issue and a one-line summary.
