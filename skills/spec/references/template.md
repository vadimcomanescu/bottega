...
`docs/specs/<YYYY-MM-DD>-<feature-slug>.md`. Contract prose holds a one-page ceiling. Section order is fixed; a section with nothing true to say is omitted, never padded. Scenario text lives only in `features/*.feature` — the doc points at it, the gate doc carries its `gate-render` walkthrough.
...
- The gate doc's scenario blocks are `gate-render.mjs` output verbatim, never hand-written — the user reads plain walkthroughs, the repo keeps the Gherkin, and `gate-diff` proves they're the same thing at sign.
