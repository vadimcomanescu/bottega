...
3. The feature file is mutation-tested: run on a copy, never the signed file (the tool writes into the file it reads), with the kit's `aps-adapter <test-command>` worker as the runner. Exit 1 means surviving mutants to judge, not a broken tool; each survivor is killed or justified in the PR.
