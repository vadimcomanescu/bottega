...
**Proof is priced.** Every job pays the floor: isolation on its own branch and worktree, a build, the host's own gates green, cross-family review, a PR. Everything above the floor — a signed Gherkin contract (with its acceptance toolchain, installed only then), storyboards, a QA drive with recordings, a docs seat, a cold read, mutation — is bought by a named risk and disclosed at the top of the PR. An artifact that protects bottega's narrative rather than the product doesn't ship.
