...
Guru ([`skills/guru`](skills/guru/SKILL.md)) runs the same delivery bar as one self-contained prompt: unknowns found and settled, a run contract and failing tests on the branch in place of the committed spec and plan, every fan-out a dynamic workflow, and its own looped two-critic review on the worker table's critic rows. It uses no other skill, reusing only the worker table and the codex dispatch method, and the contract is deleted before the PR opens: by then its content is the tests, the PR body, and at most one decision record (`docs/adr/0026-guru-one-goal-entry-point.md`).
