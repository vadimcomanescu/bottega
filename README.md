...
Guru ([`skills/guru`](skills/guru/SKILL.md)) runs the same delivery bar as one goal-driven prompt: the discover, open, build, panel, and QA skills as building blocks, a run contract and failing tests on the branch in place of the committed spec and plan, every fan-out a dynamic workflow, and its own looped two-critic review on the worker table's critic rows. It stands beside maestro with no dependency on it, and the contract never merges: by close its content is the tests, the PR body, and at most one decision record (`docs/adr/0026-guru-one-goal-entry-point.md`).
