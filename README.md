...
Guru ([`skills/guru`](skills/guru/SKILL.md)) runs the same delivery bar as one standalone prompt: the repo's own map and glossary read first, then cheap sweeps outward for how others solved this and what the installed versions actually do, unknowns found and settled, a run contract on the branch in place of the committed spec and plan, builders fanned out with the implementation doctrine in their brief, a critic drive per user-facing slice, and a looped final review by a Claude and a codex critic. It opens and closes through the same `bottega:open` and `bottega:close` maestro uses; beyond those and `bottega:panel` for a costly decision it uses no other skill, it names its workers' models where it dispatches them, and the contract is deleted before the PR opens, once the PR body carries its decisions (`docs/adr/0026-guru-one-goal-entry-point.md`, amended by `docs/adr/0030-guru-routes-its-opening-and-closing.md`).
