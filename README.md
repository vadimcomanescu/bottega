...
4. Models the domain, writes the plan as a file committed on the run branch, cuts vertical slices inside it, and puts each costly decision (where behavior or state belongs, data shape, public contracts, dependency bets) to a panel before building. A fresh GPT editor then reads the spec, the plan, and the repository cold and approves the plan or rewrites it whole; the orchestrator takes the changes it accepts and commits, and each slice's status is committed into the plan as slices land.
