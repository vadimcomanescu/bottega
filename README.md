...
Nothing else is assumed about the project. A run leaves nothing behind but the PR, the spec and plan a full-flow run commits to `docs/specs/` and `docs/plans/`, and the permanent branch holding QA evidence: working state is the worktree and one gitignored owner file, both removed at delivery.
...
**The PR is the only path to trunk.** Every run builds on its own branch in its own worktree; the user's checkout is never touched, and the run's PR reaches ready only after the integrated review, QA, and the project's checks are green on the exact head being delivered. Why: verification is the run's, authorization is the user's, and the user exercises it at launch: land on green, or hold (`docs/adr/0027-the-launch-decides-the-release.md`). The project's own machinery lands a green PR; a held PR carries the `hold` label and waits for the user to remove it; the run itself never merges, arms auto-merge, or approves. Any further requirement only a person can clear, a required review or a label, is reported the same way, the PR left open (`docs/adr/0017-close-respects-human-gates.md`).
