...
Guru ([`skills/guru`](skills/guru/SKILL.md)) runs the same delivery bar as one self-contained prompt: unknowns found and settled, a run contract on the branch in place of the committed spec and plan, every fan-out a dynamic workflow, and its own looped two-critic review on the worker table's critic rows. Beyond `bottega:panel` for costly decisions it uses no other skill, reusing only the worker table and the codex dispatch method, and the contract is deleted before the PR opens, once the PR body carries its decisions (`docs/adr/0026-guru-one-goal-entry-point.md`).
...
The user takes part in two places: discovery, and agreeing the spec.
