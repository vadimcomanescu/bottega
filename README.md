...
5. Dispatches builders with one assigned slice, the fixed architecture, the glossary, and relevant technology skills. Builders work test-first and stop at the slice boundary; the project's gates stay green at every integrate. A slice that changes a user-facing surface is driven through the real interface by a fresh worker before it merges.
...
**QA owns the product drive.** Builders prove their slice through code and tests, and a slice that changes a user-facing surface is driven through the real interface before it merges, so a defect a user would see is found before the review reads the diff. Reviewers inspect the integrated code and architecture. Only after the orchestrator accepts the review evidence does a fresh QA worker drive the accepted head and record the verdict; QA never edits product code. Every drive reports its divergences in one batch, including defects found outside the scenarios it was given, so one repair cycle clears the set. The orchestrator reads a failure before routing it, and any product-code repair gets fresh review, orchestrator acceptance, and QA. Evidence is read from the PR, never in local folders: each scenario's walkthrough gif plays in the browser from its blob page in the private evidence repository, one click from the PR body, with the full recording linked beside it (`docs/adr/0009-qa-evidence-repository.md`).
