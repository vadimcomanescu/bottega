...
## `agents/codex-rescue.md` and the codex companion runtime beside it

The mechanics every codex dispatch runs on. The rescue agent forwards one task to `scripts/codex-companion.mjs`, which drives the codex app-server through a broker, keeps job state and threads on disk per workspace, and tears down with the session.
...
- Prose, taken with recorded local scoping: `agents/codex-rescue.md`, `skills/codex-cli-runtime/SKILL.md`, and `skills/gpt-5-4-prompting/` (its `SKILL.md` and `references/codex-prompt-recipes.md`). The scoping edits, and nothing else: every `codex:codex-rescue` reads `bottega:codex-rescue`, since the agent installs under bottega's namespace; `skills/codex-cli-runtime/SKILL.md` forwards `--background` to `task` instead of stripping it (upstream strips it as Claude-side control, and bottega's watch mechanic needs the receipt a background job returns), forwards `--cwd` the same way, and lists the effort values the patched script accepts; `agents/codex-rescue.md` gains the matching one-line rule naming `--background` and `--cwd` as pass-through runtime controls.
