...
The domain model for what makes a skill great. A skill exists to make a stochastic system behave the same way every run, so **Predictability** is what every term below serves. This is the disclosed reference for [`write-bottega-skills`](SKILL.md).
...
The ordered actions the agent performs: when a skill has them, the primary tier of its content, and the part that earns its place in SKILL.md. Not every skill has steps: a skill can be all steps (`tdd`), all **reference** (a review), or both, independent of invocation. Every step carries a **finish line**, clear or vague, in its own prose.

_Avoid_: workflow, instructions, choreography

### Reference

Material the agent refers to on demand: definitions, facts, parameters, examples, conditional instructions. When a skill has **steps** it is secondary to them. When a skill has none it is the entire content. It can also live outside any skill entirely (see **External Reference**). Reached via **context pointers**, and the prime candidate for **progressive disclosure**.
...
The condition that tells the agent a unit of work is done: the target it judges against, stated in the step's own sentences rather than as a trailing formula. Two properties make it a lever, not just a quality. Its _clarity_ (can the agent tell done from not-done?) resists **premature completion**: a vague finish line ("understanding reached") lets the agent declare done and slip to the next step. That axis needs _steps_ to bite, since premature completion is a between-steps failure. Its _demand_ (how much it requires) sets **legwork**: "every modified model accounted for" forces thorough work where "produce a change list" does not. The demand axis is _not_ step-bound: it can bind a body of flat reference too, which is how a skill with no steps still carries an exhaustiveness bar ("every rule applied"). The strongest finish lines are both checkable and exhaustive.

_Avoid_: completion criterion, done condition, exit condition, stopping rule
