# Run 0001: the commission lock CLI

**Status:** closed · **Acceptance:** was `features/commission-lock.feature` (delivered 2026-07; the CLI it delivered was removed on 2026-07-08; the lock never held more than git and PR review already hold, see the simplify PR)

## Intent

Bottega's first mechanical guarantee: once the user signs a commission, the feature files *are* the contract and nothing may change them unnoticed. This run delivers the `bottega` CLI (`sign` freezes the contract, `verify` polices it), the guard every downstream gate calls.
