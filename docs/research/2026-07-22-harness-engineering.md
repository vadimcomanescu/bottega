...
This note separates three things: what the corpus argues, what bottega already does, and what a session proposed changing. Nothing here is agreed. The proposals are findings from one reading, kept so a later session inherits the reasoning instead of repeating it.
...
- Independent evidence as the reason for a second context window is the cross-family integrated review in [`skills/autoreview`](../../skills/autoreview/SKILL.md) and the independent drafts in [`skills/panel`](../../skills/panel/SKILL.md).
