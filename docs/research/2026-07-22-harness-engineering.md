...
**4. Requalify worker doctrine when the model table changes.** The corpus holds the worker constant for an epoch and requalifies on every upgrade, including subtraction: scaffolding that helped one worker becomes dead weight for the next. Bottega has the static form of this test in `AGENTS.md` (could the worker derive it, and would the plain model do better without the instruction) but no trigger that fires when the worker changes. Would land as one line in `AGENTS.md`: a change to the pinned worker models re-runs that test over the worker rules written to fence the replaced model's observed failures.
