...
The rule: a shell-out dispatch runs from the orchestrator's own turn: tracked background Bash with a resume chain under Claude Code, one foreground call with an explicit timeout under Codex. A subagent never holds a shell-out longer than a routine command.
