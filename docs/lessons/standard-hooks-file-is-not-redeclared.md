...
The rule: Claude Code auto-loads `hooks/hooks.json`; a manifest `hooks` key names only additional hook files under non-standard names. Re-declaring the standard path is a duplicate load.

Enforced: tests/worker-doctrine.test.ts ("parses the plugin manifest and points it at skills") asserts the manifest declares no hooks key.
