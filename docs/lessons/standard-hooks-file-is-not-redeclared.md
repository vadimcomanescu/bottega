...
The rule: Claude Code auto-loads `hooks/hooks.json`; a manifest `hooks` key names only additional hook files under non-standard names (as the Cursor manifest does with `hooks-cursor.json`). Re-declaring the standard path is a duplicate load.
