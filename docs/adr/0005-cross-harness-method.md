...
Superseded in part: Cursor was dropped as a target in 0.78.0 (`0015-cursor-dropped.md`), the proxy path was re-declined in 0.80.0 (`0008-model-proxy-re-declined.md`), the routing skill was dissolved into per-phase pins (`0018-pins-at-the-dispatch-sites.md`), and Claude Code became the one orchestrator (`0019-claude-code-orchestrates.md`).
...
## Context
Agent definition files are the one packaging surface with no cross-harness equivalent, and most dispatches never used them. The observed cost failures (silent fable inheritance) are prevented by the explicit-model rule alone.
