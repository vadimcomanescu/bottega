...
## Decision
Bottega's method lives entirely in skills that run under Claude Code, Codex, and Cursor. Worker identities as agent files, the entry guard, the codex-exec dispatch script, and the per-seat routing table are removed. Model selection moves to a routing skill with a flat registry; enforcement shrinks to one portable rule (a run-owning session names a model on every worker start, never fable); GPT models inside Claude Code arrive natively through an optional local proxy, with the codex CLI as fallback.
