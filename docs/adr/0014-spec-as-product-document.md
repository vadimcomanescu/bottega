...
The current shape landed in 0.63.0: the spec is a product document with a fixed floor (`skills/spec/references/spec-format.md`, removed with the spec skill by ADR 0033), written in product language, presented as a live shared document the owner reviews in comment threads, with the local markdown file as the single source of truth. Three observed failures drove it: delivered specs read as insider shorthand, models built custom solutions where standard ones existed (the format now demands the standard-solution citation), and owners skimmed specs presented as long chat messages.
