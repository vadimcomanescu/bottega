...
The harness-engineering corpus (https://github.com/lopopolo/harness-engineering at `226c8d3`) was read whole against bottega. Adopted, each verified against a real product repo before landing: a new rule fixes the in-scope violations of the class it names and files one issue for the rest (maestro's QA phase); severity gates the review loop, only a blocking finding forces a rerun (`skills/autoreview`); the builder reports what the plan, spec, or map should have told it, corroborated by the orchestrator before anything persists (`skills/implementing`); a model-table move to a new generation re-runs the worker-rule tests (AGENTS.md); builders iterate against focused checks with full gates once at the end; setup verifies boot from a fresh worktree and drafts the critical-journeys doc.
