...
| `skills/discover/SKILL.md` | The discovery method a run's Discover phase uses whole: read the code the request touches to state its intent, then an Opus sweep briefed from that reading, one worker per question the intent raised, four of them standing (the surrounding code, prior art, the installed libraries' own docs and source, the runtime's agent skills), then the field guide's moves (blind spot pass, brainstorms and prototypes, interviews where every question carries its explanation, references), ending with the direction chosen, every decision answered, and the prototypes kept for the build. Not user-invocable | Anything touches how a run states a request's intent, finds unknowns, sets scope, brainstorms, prototypes, interviews the user, or gathers references |
