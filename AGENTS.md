...
| `skills/guru/SKILL.md` | `/bottega:guru`, the whole method as one standalone prompt: a live run contract in place of the committed spec and plan, the builder doctrine carried in each brief, a critic drive per user-facing slice, `bottega:panel` its only skill route, and its workers' models named where it dispatches them | Anything touches the guru run's method, its contract, or its critics |
...
| `skills/build/SKILL.md` | The build method a run's Build phase uses whole: dispatch builders per slice, check reports, integrate with gates green, drive every landed slice that changes a user-facing surface, end with the simplification pass; not user-invocable | Anything touches how slices are dispatched, checked, integrated, driven, or simplified |
| `skills/open/SKILL.md` | The opening method a run's Open phase uses whole: ownership, worktree and branch, the owner file and release answer, commands from the agent map; not user-invocable | Anything touches how a run starts or claims work |
| `skills/spec/SKILL.md` | The spec method: write the agreed spec from discovery's findings, present it live, and commit it; `references/spec-format.md` sets the document shape, and `references/live-review.md` carries the live-document review mechanics | Anything touches spec writing, the live review, or the agreed spec's publish |
| `skills/plan/SKILL.md` | The plan method a run's Plan phase uses whole: model the domain, commit the plan on the run branch, strengthen it with one cold read by a GPT editor; not user-invocable | Anything touches how a plan is modeled, committed, or strengthened |
...
| `skills/qa/SKILL.md` | QA method and limits: drive the shipped interface, return PASS/FAIL/NOT VERIFIED per scenario, forbidden actions; the run's QA phase and a landed slice's drive route here | Anything touches QA driving, evidence, or limits |
...
| `skills/panel/SKILL.md` | `/bottega:panel`, independent drafts from different companies' models and a compare-only judge for one costly decision; the run's Discover and Plan phases call it too, and it states the three conditions that make a panel worth its cost | Changing panel seats, judging, or when a panel is worth running |
...
| `skills/calibrate/SKILL.md` | `/bottega:calibrate`, sentence-level audit of a repo's agent docs against the Claude 5 calibration bar; the dated research note under `docs/research/` carries its sources | Changing the calibration bar or how the audit proposes and applies cuts |
...
| `docs/lessons/` | Failure records: what happened, the rule, and where the rule is enforced | Shaping new work in discovery or plan, and when a run diagnoses a failure worth keeping |
...
- Direct authoring keeps a run's documentation duties. In the same diff: a decision meeting the ADR bar (`bottega:codebase-design`) lands its ADR, a diagnosed failure lands its lesson in `docs/lessons/`, and every doc a change makes stale is fixed or deleted. The documentation duties this repo preaches to host repos govern this repo; the release differs: bottega arms no auto-merge of its own, so a PR the owner merges delivers a bottega change until this repo adopts `docs/adr/0028-the-launch-decides-the-release.md`.
