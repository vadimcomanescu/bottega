...
| `skills/signoff` | User gate — the collaborative spec doc (storyboards inline, comment loop, `SIGNED` as a comment); `assets/` holds the local canvas for at-the-machine sessions, `gate-render.mjs`, the deterministic feature-file→walkthrough renderer whose output is what the user signs, and `gate-diff.mjs`, the sign-blocking check that the hosted doc carries that output verbatim |
...
| `tests/` | Unit tests for the hooks, the walkthrough renderer, and the gate-diff check |
...
- Verification gate: `npm test` (hook + gate-render + gate-diff unit tests). A host run's delivery gate is the acceptance run plus the acceptance mutation run, survivors killed or justified in the PR body.
