...
| `skills/land/SKILL.md` | Takes an open PR through review-fix rounds to verified-mergeable (`/bottega:land`); merges only when the request armed it. Owns the GitHub surface (review threads via `scripts/pr-threads`), the stop conditions, the terminal notification, and the merge verification |
