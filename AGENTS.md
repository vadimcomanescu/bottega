...
| `skills/guru/SKILL.md` | `/bottega:guru`, the whole method as one self-contained prompt: a live run contract and failing tests in place of the committed spec and plan, critics on the worker table's critic rows, no other skill used, the contract deleted before the PR opens | Anything touches the guru run's method, its contract, or its critics |
