...
| `skills/build/SKILL.md` | The build method a run's Build phase uses whole: dispatch builders per slice, check reports, drive every slice that changes a user-facing surface, integrate with gates green, end with the simplification pass; not user-invocable | Anything touches how slices are dispatched, checked, driven, integrated, or simplified |
...
| `skills/qa/SKILL.md` | QA method and limits: drive the shipped interface, return PASS/FAIL/NOT VERIFIED per scenario, forbidden actions; the run's QA phase and a slice's pre-merge drive route here | Anything touches QA driving, evidence, or limits |
