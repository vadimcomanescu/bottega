...
| `skills/maestro/SKILL.md` | `/bottega:maestro`, the whole run as one prompt in the owner's own words: the release answer and the model check, discovery, the design and its slices, the codex second opinion, autonomous builders with a fresh reviewer each, the integrated review with its spec and architecture reads, the QA loop, close; every worker's model is named in the sentence that dispatches it | Anything touches a run's flow, its state, or which model runs where |
