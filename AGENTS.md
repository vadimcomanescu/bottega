...
| `skills/panel/SKILL.md` | `/bottega:panel`, independent drafts from different companies' models and a compare-only judge for one costly decision; the run's Spec and Plan phases call it too, and it states the three conditions that make a panel worth its cost | Changing panel seats, judging, or when a panel is worth running |
