...
- Where a role's instructions live: a skill is method consumed by more than one role or more than one runtime; method small enough to ride in a dispatch (QA, the docs sweep) lives in `skills/run` and travels in the brief. Agent files point, never summarize.
