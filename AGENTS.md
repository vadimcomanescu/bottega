...
| `skills/execute/SKILL.md` | Maestro run doctrine (`/bottega:execute`) — expects a locked, unexecuted spec and refuses anything else; architecture authority, routing, the review loop, delivery, the spec close-out |
...
| `hooks/` | Route guard (PreToolUse) — while a commission is forming or in flight, rejects **any** dispatch that omits `model` or names fable (cold read passes by naming itself); entry guard (UserPromptSubmit) points run-intent prose at the three entry commands. The fable fence, harness-enforced |
...
| `.bottega/` | Runtime: `aps.lock` (pinned toolchain), `verify/<sha>/` (evidence), `wt/<slice>/` (worktrees), `run/records/` + `evidence/` (per-slice run records and worker evidence, audited by `verify --delivery`) |
...
- Verification gate: `npm test` (unit + generated acceptance) and, at delivery, `bottega verify --delivery` (run-record audit) plus the acceptance mutation run with survivors killed or justified in `.bottega/verify/<sha>/equivalent-mutants.json`.
