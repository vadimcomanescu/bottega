...
| `skills/review/SKILL.md` | The review method a run's Review phase invokes whole: the doc-coverage check, the blind autoreview panel, and the spec-conformance check, rerun to convergence at one head; not user-invocable | Anything touches the review phase's sequencing, the conformance check, or how a missed defect becomes a review-doctrine rule |
| `skills/autoreview` | The review engine: upstream's autoreview document locally adapted (its `AGENTS.md` carries the sync contract), plus the vendored helper scripts and tests; see README Credits | Anything touches review mechanics, inputs, fix dispatch, PR threads, or merge verification |
