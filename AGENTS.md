...
| `skills/guru/SKILL.md` | `/bottega:guru`, the whole method as one goal-driven prompt: the discover, open, build, panel, and QA skills as building blocks, a live run contract and failing tests in place of the committed spec and plan, critics on the worker table's critic rows, no dependency on maestro | Anything touches the guru run's method, its contract, or its critics |
