...
| `skills/land/SKILL.md` | Takes an open PR through review-fix rounds to mergeable (`/bottega:land`). Owns the GitHub surface (inline comments and review threads via `scripts/pr-threads`), the stop conditions, the terminal notification, and the merge policy |
